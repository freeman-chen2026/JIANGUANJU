...
def run_feature_e():
    ...
    if uploaded_file:
        ...
        if file_type in ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel"]:
            ...
            if not matches:
                # 兜底：按行扫描
                st.warning("未找到日期标记，尝试按行扫描...")
                day_pattern = re.compile(r'(\d+)日')
                for line in lines:
                    if "日" in line and ("白" in line or "晚" in line):
                        day_match = day_pattern.search(line)
                        if day_match:
                            day_num = int(day_match.group(1))
                            shift = "白" if "白" in line else "晚"
                            date_str = f"9月{day_num}日"
                            # 提取姓名
                            name_pattern = re.compile(r'[\u4e00-\u9fa5]{2,4}')
                            names = name_pattern.findall(line)
                            exclude = {"运行", "控制", "带班", "主任", "计划", "监控", "保障", "支援", "管理", "工作", "时间", "调整", "参与", "席位", "安排", "次日", "白班", "夜班", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"}
